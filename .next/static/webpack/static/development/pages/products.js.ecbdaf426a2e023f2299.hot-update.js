...
function Filter_popup_more() {
  ...
  if (filter_accum !== '') {
    ...
    console.table(url_arr[0]);
    console.table(url_arr[1]);
    console.table(url_arr[2]); // var assemble_url_var = url_arr.map(item => (assemble_url_fun(item)));
    ...
    var assemble_url_filters = url_arr[2] ? "filters=".concat(url_arr[2].map(function (post) {
      return '&filters=' + post.node_label;
    }).join('')) : '';
    assemble_url_var = assemble_url_genders.concat(assemble_url_ages, assemble_url_filters).join('');
    console.log(assemble_url_var); // var assemble_url = assemble_url_genders
...
//# sourceMappingURL=products.js.ecbdaf426a2e023f2299.hot-update.js.map