...
function Filter_popup_more() {
  ...
  for (var i = 0; i < filter_var.length; i++) {
    if (filter_var[i].checked) {
      filter_accum = filter_accum + filter_var[i].parentElement.querySelector('span').innerHTML.trim() + ' ';
    }
  }

  if (filter_accum !== '') {
    filter_accum = encodeURI(filter_accum.toLowerCase().trim());
    next_router__WEBPACK_IMPORTED_MODULE_7___default.a.push('/products?gender=&gender=female&gender=male&age=&age=children&age=adult&filters=' + filter_accum);
  } else {
...
function gen_HTML_for_filter_input(fields, url_params, type_) {
  return Object.values(fields).map(function (item) {
    return "<label><input type=\"".concat(type_, "\" ").concat(url_params != null && url_params.includes(item) ? 'checked' : '', "/>\t<span>").concat(item, "</span></label>");
  });
}
...
function fetch_products() {
  ...
  axios__WEBPACK_IMPORTED_MODULE_6___default.a.get(_components_Vars__WEBPACK_IMPORTED_MODULE_4__["default"] + '/wp-json/global-custom-types/v1/settings').then(function (response) {
    document.getElementById('fetch_types').innerHTML = gen_HTML_for_filter_input(response.data.types.choices, url_params, 'radio').toString().replace(/,/g, '');
    document.getElementById('fetch_genders').innerHTML = gen_HTML_for_filter_input(response.data.genders.choices, url_params, 'checkbox').toString().replace(/,/g, '');
    document.getElementById('fetch_ages').innerHTML = gen_HTML_for_filter_input(response.data.ages.choices, url_params, 'checkbox').toString().replace(/,/g, '');
...
//# sourceMappingURL=products.js.895d2682c2a4daa24587.hot-update.js.map