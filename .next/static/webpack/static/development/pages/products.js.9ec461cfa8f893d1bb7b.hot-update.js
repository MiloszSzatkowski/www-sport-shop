...
function gen_HTML_for_filter_input(fields, url_params, type_, parent_type) {
  return Object.values(fields).map(function (item) {
    return "<label class=\"".concat(parent_type, "\"><input type=\"").concat(type_, "\" name=\"").concat(parent_type, "\" ").concat(url_params != null && item.includes(url_params) ? 'checked' : '', "/>\t<span>").concat(item, "</span></label>");
  }).toString().replace(/,/g, '');
}

function fetch_products() {
  var url_params_filters = new URL(window.location.href).searchParams.get("filters");
  var url_params_genders = new URL(window.location.href).searchParams.get("gender");
  var url_params_ages = new URL(window.location.href).searchParams.get("age");
  console.log(url_params_ages);
...
//# sourceMappingURL=products.js.9ec461cfa8f893d1bb7b.hot-update.js.map